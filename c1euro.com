...
<!-- JS d'ouverture / fermeture / tracking Umami amélioré -->
<script>
  window.addEventListener('load', function () {
    ...
    // Fonction de tracking robuste avec retries
    function trackUmami(eventName, retries = 10) {
      if (typeof window.umami === 'function') {
        window.umami(eventName);
        console.log(`[UMAMI] Événement envoyé : ${eventName}`);
      } else if (retries > 0) {
        console.log(`[UMAMI] En attente de chargement pour ${eventName}... Tentatives restantes : ${retries}`);
        setTimeout(() => trackUmami(eventName, retries - 1), 500);
      } else {
        console.warn(`[UMAMI] Échec de l'envoi : ${eventName}`);
      }
    }
    ...
    // Logic d'affichage conditionnel
    (function () {
      const now = Date.now();
