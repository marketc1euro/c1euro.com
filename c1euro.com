...
});</script><p><script defer="defer" src="https://cloud.umami.is/script.js" data-website-id="edb4f2a1-5cdf-4ef3-b837-efe910763b4b"></script></p><div id="popup" style="position: fixed; inset: 0; background: #161718; display: none; justify-content: center; align-items: center; z-index: 99999;"><div style="background: #161718; width: 100%; height: 100%; padding: 30px; overflow-y: auto; position: relative; display: flex; flex-direction: column; justify-content: center; align-items: center;"><button id="popup-close" style="position: absolute; top: 15px; right: 20px; width: 36px; height: 36px; background: #ddd; border: none; border-radius: 50%; font-size: 24px; cursor: pointer; color: #111; display: flex; justify-content: center; align-items: center; box-shadow: none; line-height: 1; padding: 0; user-select: none;" title="Fermer" aria-label="Fermer">×</button><script src="https://unpkg.com/lottie-web@5.10.1/build/player/lottie.min.js"></script><div id="lottie-container" style="width: 300px; height: 300px; margin-bottom: 20px;"></div><script>lottie.loadAnimation({
        container: document.getElementById('lottie-container'),
        renderer: 'svg',
        loop: false,
        autoplay: true,
        path: '/media/files/mobile.json'
      });</script><div style="display: flex; justify-content: center; width: 100%; margin-bottom: 25px;"><div style="display: flex; gap: 15px; overflow-x: auto; padding: 10px 0; max-width: 100%;"><div style="flex: 0 0 140px; background: #242528; border-radius: 8px; padding: 12px; text-align: center;"><img style="display: block; width: 80px; height: 80px; margin: 0 auto 8px;" src="https://c1euro.com/media/posts/15/4cdecc1f11ba304d07d53eac145691a045585410-80x80_Z1T7HQb-2.svg" alt=""><div style="background: #161718; color: #fff; border-radius: 999px; padding: 4px 12px; font-size: 0.85em; display: inline-block; user-select: none;">Mode</div></div><div style="flex: 0 0 140px; background: #242528; border-radius: 8px; padding: 12px; text-align: center;"><img style="display: block; width: 80px; height: 80px; margin: 0 auto 8px;" src="https://c1euro.com/media/posts/15/7c505ae37f53d46df50d526fc771b0084c867e64-80x80_16paFn-1.svg" alt=""><div style="background: #161718; color: #fff; border-radius: 999px; padding: 4px 12px; font-size: 0.85em; display: inline-block; user-select: none;">Alimentation</div></div><div style="flex: 0 0 140px; background: #242528; border-radius: 8px; padding: 12px; text-align: center;"><img style="display: block; width: 80px; height: 80px; margin: 0 auto 8px;" src="https://c1euro.com/media/posts/15/700c06cf291d4a213547eb48733aea2422f06f87-80x80_ZNVBu0.svg" alt=""><div style="background: #161718; color: #fff; border-radius: 999px; padding: 4px 12px; font-size: 0.85em; display: inline-block; user-select: none;">High-tech</div></div><div style="flex: 0 0 140px; background: #242528; border-radius: 8px; padding: 12px; text-align: center;"><img style="display: block; width: 80px; height: 80px; margin: 0 auto 8px;" src="https://c1euro.com/media/posts/15/b430f69d910f82bf07dca72bc21a49aa5887710c-80x80_Zhczk2-1.svg" alt=""><div style="background: #161718; color: #fff; border-radius: 999px; padding: 4px 12px; font-size: 0.85em; display: inline-block; user-select: none;">Livreur</div></div></div></div><div style="text-align: center; font-size: 1.2em; margin-bottom: 15px; font-family: 'Public Sans', sans-serif; color: #000;"><span style="color: #95a5a6;">Cliquez ci-dessous pour télécharger l'application mobile</span></div><p style="text-align: center; margin-top: 0; margin-bottom: 5px;"><a href="https://link.c1euro.com/qrcode" id="launch-app-btn" style="display: inline-block; padding: 14px 28px; background-color: #4217f4; color: #fff; text-decoration: none; border-radius: 9999px; font-size: 16px; font-weight: 600; font-family: 'Public Sans', sans-serif;">Lancer l'application</a></p></div></div><p><script>// Fermeture popup
  document.getElementById('popup-close').addEventListener('click', function () {
    document.getElementById('popup').style.display = 'none';
  });

  // Event Umami au clic sur "Lancer l'application"
  document.getElementById('launch-app-btn').addEventListener('click', function () {
    if (window.umami) {
      window.umami('popdownload');
    }
  });

  // Gestion automatique ouverture popup avec stockage localStorage
  (function() {
    ...
    // Récupérer timestamps depuis localStorage
    let firstOpenTime = localStorage.getItem('popup_first_open') ? parseInt(localStorage.getItem('popup_first_open')) : null;
    let secondOpenTime = localStorage.getItem('popup_second_open') ? parseInt(localStorage.getItem('popup_second_open')) : null;
    let lastOpenTime = localStorage.getItem('popup_last_open') ? parseInt(localStorage.getItem('popup_last_open')) : null;
    ...
    function tryOpenPopup() {
      ...
      if (firstOpenTime && !secondOpenTime) {
        if (now - firstOpenTime >= 20000) {
          openPopup();
      ...
      if (secondOpenTime) {
        if (!lastOpenTime || now - lastOpenTime >= 180000) {
          openPopup();
          updateTimes('popup_last_open', now);
        }
      }
