...
<!-- SCRIPT UMAMI complet avec délai 30s -->
<script>
  window.addEventListener('load', function () {
    ...
    // Tracking d'affichage du popup
    if (window.umami) {
      umami.track('popup-view');
    }

    // Tracking au clic avec gestion mobile/PC
    launchAppBtn.addEventListener('click', function (e) {
      ...
      if (!isMobile) {
        e.preventDefault();
        location.reload(); // Reste sur la même page
      }
    });
    ...
    // Affichage conditionnel (localStorage + délais) AVEC DÉLAI INITIAL 30s
    (function () {
      const now = Date.now();
      ...
      function tryOpenPopup() {
        if (!firstOpenTime) {
          // Délai initial de 30 secondes
          setTimeout(() => {
            openPopup();
            updateTimes('popup_first_open', Date.now());
            updateTimes('popup_last_open', Date.now());
          }, 30000);
          return;
        }
