...
@media (max-width: 1024px) {
  ...
  });</script><script src="https://cdnjs.cloudflare.com/ajax/libs/bodymovin/5.10.1/lottie.min.js"></script><script>// À placer juste avant 
document.addEventListener('DOMContentLoaded', function() {
  const attopImages = document.querySelectorAll('.content__featured-image--attop img');
  const otherImages = document.querySelectorAll('figure.content__featured-image:not(.content__featured-image--attop) img');

  // 1. Optimisation spécifique pour les images --attop (sans casser le design)
  attopImages.forEach(img => {
    if (window.innerWidth >= 1024) {
      // Solution light pour la netteté
      img.loading = 'eager';
      img.style.imageRendering = '-webkit-optimize-contrast';
      
      // Alternative si problème avec transform
      img.style.backfaceVisibility = 'hidden';
      img.style.transform = 'perspective(1px)';
    }
  });

  // 2. Optimisation standard pour les autres images
  otherImages.forEach(img => {
    img.loading = 'lazy';
    img.decoding = 'async';
  });

  // 3. Recalcul des dimensions après chargement (anti-layout shift)
  window.addEventListener('load', function() {
    attopImages.forEach(img => {
