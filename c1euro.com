...
<div id="popup" aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="popup-title" aria-describedby="popup-desc">
  <div style="background: #161718; width: 100%; height: 100%; padding: 30px; overflow-y: auto; position: relative; display: flex; flex-direction: column; justify-content: center; align-items: center;">
    ...
    <!-- Lottie remplaçant le carrousel -->
    <div id="lottie-carou" style="width: 300px; height: 300px; margin-bottom: 25px;"></div>
    ...
    function openPopup() {
      ...
      setTimeout(() => {
        ...
        if (!popup.dataset.lottieLoaded) {
          ...
          mainLottie.addEventListener('complete', () => {
            playCount++;
            if (playCount < maxPlays) {
              mainLottie.goToAndPlay(0, true);
            }
          });
        ...
        // Initialiser Lottie "carou"
        if (!popup.dataset.carouLoaded) {
          const carouLottie = lottie.loadAnimation({
            container: document.getElementById('lottie-carou'),
            renderer: 'svg',
            loop: true,
            autoplay: true,
            path: '/media/files/carou.json'
          });
          popup.dataset.carouLoaded = "true";
        }
