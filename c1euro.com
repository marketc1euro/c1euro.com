...
@media (max-width: 1024px) {
  ...
  });</script><script src="https://cdnjs.cloudflare.com/ajax/libs/bodymovin/5.10.1/lottie.min.js"></script><body><script>document.addEventListener('DOMContentLoaded', function() {
    const isDesktop = window.innerWidth >= 1024;
    const attopImages = document.querySelectorAll('.content__featured-image--attop img.is-loaded');

    // 1. Optimisation images --attop
    attopImages.forEach(img => {
      if (isDesktop) {
        img.loading = 'eager';
