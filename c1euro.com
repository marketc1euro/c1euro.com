...
@media (max-width: 1024px) {
  ...
  });</script><script src="https://cdnjs.cloudflare.com/ajax/libs/bodymovin/5.10.1/lottie.min.js"></script><script>// Optimisation automatique pour toutes les pages
function optimizeLCPImage() {
  // 1. Cible l'image LCP (adaptez le sélecteur si nécessaire)
  const lcpImage = document.querySelector('.content__featured-image--attop img.is-loaded');
  
  if (!lcpImage) return;

  // 2. Paramètres de netteté extrême
  lcpImage.style.cssText = `
  ...
  if (!document.querySelector('link[as="image"]')) {
...
if (document.readyState === 'complete') {
  ...
  window.addEventListener('load', optimizeLCPImage);
