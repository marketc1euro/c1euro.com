...
@media (max-width: 1024px) {
  ...
  // ✅ On récupère toutes les images visibles
  const images = document.querySelectorAll('img');

  if (images.length > 0) {
    // ✅ On prend la première image (souvent la plus importante)
    const firstImage = images[0];

    // ✅ On la rend prioritaire
    firstImage.setAttribute('fetchpriority', 'high');
    firstImage.removeAttribute('loading'); // enlève le lazy-loading

    console.log("✅ LCP optimisé pour :", firstImage.src);
  }
