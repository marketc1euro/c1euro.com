...
@media (max-width: 1024px) {
  ...
  });</script><script src="https://cdnjs.cloudflare.com/ajax/libs/bodymovin/5.10.1/lottie.min.js"></script><script>// Solution hybride : Netteté + Performance (à placer avant)
function handleCriticalImage() {
  const lcpImage = document.querySelector('.content__featured-image--attop img.is-loaded');
  if (!lcpImage) return;

  // 1. VOTRE TECHNIQUE ANTI-FLOU (conservée intégralement)
  if (window.innerWidth >= 1024) {
    lcpImage.removeAttribute('srcset');
    lcpImage.style.imageRendering = 'crisp-edges';
  }

  // 2. Optimisations LCP ajoutées (sans interférence)
  lcpImage.loading = 'eager';
  lcpImage.decoding = 'sync';
  
  // 3. Préchargement Cloudflare (nouveau)
...
if (document.readyState === 'complete') {
  ...
  window.addEventListener('load', handleCriticalImage);
  window.addEventListener('resize', handleCriticalImage); // Conserve votre gestion du resize
