...
<script>
  ...
  (function () {
    ...
    function shouldOpenAfter(ref, minutes) {
      return !ref || (Date.now() - ref >= minutes * 60 * 1000);
    }

    if (!firstOpen && shouldOpenAfter(null, 0.5)) {
      setTimeout(() => {
        showPopup();
        localStorage.setItem(FIRST_OPEN_KEY, Date.now());
        localStorage.setItem(LAST_OPEN_KEY, Date.now());
      }, 30 * 1000);
    } else if (firstOpen && !secondOpen && shouldOpenAfter(firstOpen, 2)) {
      showPopup();
      localStorage.setItem(SECOND_OPEN_KEY, Date.now());
      localStorage.setItem(LAST_OPEN_KEY, Date.now());
    } else if (secondOpen && shouldOpenAfter(lastOpen, 3)) {
      showPopup();
      localStorage.setItem(LAST_OPEN_KEY, Date.now());
    }
  })();
