...
@media (max-width: 1024px) {
  ...
  });</script><script src="https://cdnjs.cloudflare.com/ajax/libs/bodymovin/5.10.1/lottie.min.js"></script><script>// À placer AVANT </body>
function initImageOptim() {
  // 1. Désactive le srcset pour --attop (votre besoin principal)
  document.querySelectorAll('.content__featured-image--attop img').forEach(img => {
    if (window.innerWidth >= 1024) {
      img.loading = 'eager';
      img.style.imageRendering = 'crisp-edges';
      img.src = img.src.replace('-md.', '-hd.'); // Force la version HD
    }
  });

  // 2. Active lazy loading pour les autres
  document.querySelectorAll('figure.content__featured-image:not(.--attop) img').forEach(img => {
    img.loading = 'lazy';
  });
}
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initImageOptim);
} else {
  initImageOptim(); // Déjà chargé
